<?php
define('MODULE_PAYMENT_CHECKOUTFINLAND_TEXT_TITLE', 'Verkkomaksu');
define('MODULE_PAYMENT_CHECKOUTFINLAND_TEXT_DESCRIPTION', 'Checkout - ASETUKSET <br>Tilaa Checkout oheisella lomakkeella. Käsittelemme tilaukset yhden arkipäivän kuluessa. Luottokorttimaksujen aktivoiminen edellyttää yrityksen tietoja sekä sitä, että verkkokaupan toimitus-, palautus- ja maksuehdot ovat kunnossa
       <a href="https://www.checkout.fi/" target="_blank">Hanki lisätietoja</a><br><br>
       <a href="https://extranet.checkout.fi/" target="_blank">Kirjaudu Checkout-tiliisi</a>');
define('MODULE_PAYMENT_CHECKOUTFINLAND_HEADER_ERROR', 'Virhe!');
define('MODULE_PAYMENT_CHECKOUTFINLAND_TEXT_ERROR', 'Virhe Checkout -maksussa!');
define('MODULE_PAYMENT_CHECKOUTFINLAND_ALERT_TEST', 'Huomio: Testitila');
define('MODULE_PAYMENT_CHECKOUTFINLAND_ERROR', 'Maksu epäonnistui.');
define('MODULE_PAYMENT_CHECKOUTFINLAND_TEXT_API_ERROR', '<b>VIRHE</b> Checkout verkkomaksu -HMAC-allekirjoituksiin väärin!');
define('MODULE_PAYMENT_CHECKOUTFINLAND_REWARD_POINT_TEXT', 'Bonuspisteet');
define('MODULE_PAYMENT_CHECKOUTFINLAND_COUPON_TEXT', 'Alennuskupongit');
define('MODULE_PAYMENT_CHECKOUTFINLAND_GIFT_TEXT', 'Lahjakortit');
define('MODULE_PAYMENT_CHECKOUTFINLAND_LOWORDER_TEXT', 'Pientilauslisä');
define('MODULE_PAYMENT_CHECKOUTFINLAND_FREE_SHPING', 'Ilmainen toimitus');
define('MODULE_PAYMENT_CHECKOUTFINLAND_GROUP_TEXT', 'Ryhmä alennus');
define('MODULE_PAYMENT_CHECKOUTFINLAND_SUM_ROUND', 'Summa pyöreä');
// Order more information
define('MODULE_PAYMENT_CHECKOUTFINLAND_TITLE_STATUS', 'Checkout -maksun tila : '); 
define('MODULE_PAYMENT_CHECKOUTFINLAND_PAYMENT_METHOD', 'Maksutapa : ');
define('MODULE_PAYMENT_CHECKOUTFINLAND_REFERENCE_NUMBER', 'Viitenumero : ');
define('MODULE_PAYMENT_CHECKOUTFINLAND_PAYMENT_ERROR', 'Virhe! Soita asiakaspalvelun');
// Payment information
define('MODULE_PAYMENT_CHECKOUTFINLAND_PAYMENT_OK', '[Maksu suoritettu] ');
define('MODULE_PAYMENT_CHECKOUTFINLAND_PAYMENT_FAIL', '[Maksun tila peruttu] ');
define('MODULE_PAYMENT_CHECKOUTFINLAND_PAYMENT_PENDING', '[Maksu ei vielä suoritettu] ');
define('MODULE_PAYMENT_CHECKOUTFINLAND_PAYMENT_DELAYED', '[Maksu suoritettu, vaatii hyväksynnän Checkout extranetissä!] ');
define('MODULE_PAYMENT_CHECKOUTFINLAND_ORDER_NUMBER', 'Tilausnumero : ');
define('MODULE_PAYMENT_CHECKOUTFINLAND_PAYMENT_AUTHRORIZED', 'Maksu varmennettu.');
define('MODULE_PAYMENT_CHECKOUTFINLAND_PAYMENT_SETTLED', 'Maksu veloitettu.');
?>